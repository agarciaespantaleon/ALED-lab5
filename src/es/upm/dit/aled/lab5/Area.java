...
public class Area {
	...
	public Area(String name, int time, int capacity, Position2D position) {
		...
		//al principio de los tiempos inicializo a 0
		numPatients=0;
		waiting=0;
	...
	public synchronized void enter(Patient p) {
		//incremento waiting por si tengo que mandar al paciente a esperar
		waiting++;
		while(numPatients>=capacity)
			try {
				wait();
			} catch (InterruptedException e) {}
		//por si el paciente no ha tenido que esperar, le saco de waiting
		waiting--;
		numPatients++;
	 ...
	 */
	public synchronized int getNumPatientes(){
		return numPatients;
	}
